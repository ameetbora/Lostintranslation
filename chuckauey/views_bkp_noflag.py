    ...
    def get(self, request, format=None,*args, **kwargs):
        ...
        try:
            if lat and lon and day and time:
                ...
                print (lat,lon,day,time)
                query = 'SELECT 1 as id, Bay_id, Lat, Lon, St_marker_id, (6367*acos(cos(radians(%2f))*cos(radians(Lat))*cos(radians(Lon)-radians(%2f))+sin(radians(%2f))*sin(radians(Lat)))) AS distance FROM onstreet_parking_bay_sensors HAVING distance > 0.05 ORDER BY distance LIMIT 0,30' %(float(lat),float(lon),float(lat))
                signs = OnstreetParkingBaySensors.objects.raw(query)
                client = Socrata("data.melbourne.vic.gov.au", None)
                #print(signs)
                mlist = list()
                for sign in signs:
                    ...
                    find_status = client.get("dtpv-d4pf", bay_id=sign.bay_id)
                    if find_status:
